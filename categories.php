		...
		<div class="col-md-3 py-3">
			<div class="card">
				<ul class="list-group list-group-flush">
					...
					?>
						<a href=""><li class="list-group-item"><?=$subcategory['name']?></li></a>
					<?php } ?>
				</ul>
			</div>
		</div>
			...
			<div class="row">
				...
				<?php 
					foreach ($subcategories as $subcategory) {
						...
						$sql = "SELECT items.*,brands.name as brand_name, subcategories.name as sub_name FROM items INNER JOIN brands ON items.brand_id = brands.id INNER JOIN subcategories ON items.subcategory_id= subcategories.id WHERE items.subcategory_id=:sub_id";
						$stmt=$pdo->prepare($sql);
